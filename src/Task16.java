...
public class Task16 extends Task {
    ...
    public String fill() {
        if (variant == 5) {
            return  "16. Дана функция распределения F(x) непрерывной случайной\n" +
                    ...
                    "2) найти M(X), D(X), σ(Х);\n" +
                    "3) найти Р(a < X < b) для данных a = "+formatNum(a1)+", b = "+formatNum(b1)+".";
        } else {
            return "16. Дана функция распределения F(x) непрерывной случайной\n" +
                    "величины X.\n" +
                    "Требуется:" +
                    "1) найти плотность вероятности f(x);\n" +
                    "2) найти M(X), D(X), σ(Х);\n" +
                    "3) найти Р(a < X < b) для данных a = "+formatNum(a2)+", b = "+formatNum(b2)+".";
        }
    }
    ...
    public String answer() {
        if(variant==5){
            ...
            return "16. 1) f(x) = " + fx + "; 2) M(X) = " + mx + ", D(X) = " + dx + ", σ(Х) = " + sigmax + "; 3) Р(a < X < b) = " + formatNum(P) + ".";
        } else {
            ...
            return "16. 1) "+fx+"; 2) MX = "+formatNum(MX)+", DX = "+formatNum(DX)+", σ(Х) = "+formatNum(sigma)+"; 3) "+formatNum(P);
        }
    }
}
